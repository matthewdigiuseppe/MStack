...
4. Each ≤ 250 words (or the journal cap if shorter) and contains the question, the design with its estimand, the data, the finding with a number in substantive units, and the contribution (the abstract bar in `${CLAUDE_PLUGIN_ROOT}/references/writing-conventions.md`); no citations, no hedging stack; only the order varies.
